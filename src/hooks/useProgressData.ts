...
// user's sessions, then derives every chart dataset for the selected
// granularity (daily / weekly / monthly) via `computeProgress`.
//
...
import { computeProgress, type Granularity, type ProgressData } from '../utils/progressMetrics'
import type { Session } from '../types/session'

export interface UseProgressData {
  /** Computed chart datasets for the selected granularity. */
  data: ProgressData
...
export function useProgressData(granularity: Granularity): UseProgressData {
  ...
  const data = useMemo(
    () => computeProgress(sessions, granularity, new Date(), { baselineCTL, createdAt }),
    [sessions, granularity, baselineCTL, createdAt],
  )
